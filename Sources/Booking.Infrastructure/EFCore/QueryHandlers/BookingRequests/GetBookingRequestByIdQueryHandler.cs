...
internal class GetBookingRequestByIdQueryHandler : ISingleQueryHandler<BookingDbContext, GetBookingRequestByIdQuery, BookingRequest>
{
    ...
    {
        var bookingRequest = await request.Context.BookingRequests.FirstOrDefaultAsync(
            bookingRequest => bookingRequest.Id == request.Query.BookingRequestId,
            cancellationToken: cancellationToken);

        return bookingRequest ?? throw new BookingRequestNotFound($"Заявка с идентификатором [{request.Query.BookingRequestId}] не найдена.");
    }
}