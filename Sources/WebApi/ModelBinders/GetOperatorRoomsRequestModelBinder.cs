...
public class GetOperatorRoomsRequestModelBinder : IModelBinder
{
    public Task BindModelAsync(ModelBindingContext bindingContext)
    {
        ...
        var afterOperatorRoomId = ParseIntOrDefault(q["afterOperatorRoomId"], defaultValue: 0);

        OperatorRoomsFilterModel? filter = null;
        if (q.TryGetValue(key: "filter", out var filterValues) && filterValues.Count > 0)
        {
            var json = filterValues[0];
            try
            {
                filter = JsonSerializer.Deserialize<OperatorRoomsFilterModel>(json, new JsonSerializerOptions
                {
        ...
        var result = new GetOperatorRoomsModel
        {
            Page = page,
            PageSize = pageSize,
