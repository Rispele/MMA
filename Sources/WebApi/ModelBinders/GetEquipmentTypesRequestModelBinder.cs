...
public class GetEquipmentTypesRequestModelBinder : IModelBinder
{
    public Task BindModelAsync(ModelBindingContext bindingContext)
    {
        ...
        var afterEquipmentTypeId = ParseIntOrDefault(q["afterEquipmentTypeId"], defaultValue: 0);

        EquipmentTypesFilterModel? filter = null;
        if (q.TryGetValue(key: "filter", out var filterValues) && filterValues.Count > 0)
        {
            var json = filterValues[0];
            try
            {
                filter = JsonSerializer.Deserialize<EquipmentTypesFilterModel>(json, new JsonSerializerOptions
                {
        ...
        var result = new GetEquipmentTypesModel
        {
            Page = page,
            PageSize = pageSize,
