...
public class EquipmentSchemasController(IEquipmentSchemaService equipmentSchemaService) : ControllerBase
{
    ...
    public async Task<ActionResult<EquipmentSchemasResponseModel>> GetEquipmentSchemas(
        [GetRequestWithJsonFilterModelBinder<EquipmentSchemasFilterModel>]
        GetRequest<EquipmentSchemasFilterModel> model,
    ...
    {
        var model = await equipmentSchemaService.ExportEquipmentSchemaRegistry(Response.BodyWriter.AsStream(), cancellationToken);
